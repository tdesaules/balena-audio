...
# replace conf var
for SOURCE in $(echo $STREAMS | jq -r '.[].name')
do
    for STREAM in $(echo $STREAMS | jq -r ".[] | select(.name==\"$SOURCE\")" | jq -r '.streams[]')
    do
        SOURCE_STREAM="${SOURCE_STREAM}--stream.source pipe:///var/cache/snapcast/${SOURCE}_${STREAM}?name=${SOURCE}_${STREAM}&codec=pcm&sampleformat=${SOUND_RATE}:${SOUND_BIT}:2 "
    done
done

# start snapcast server
snapserver $SOURCE_STREAM &
